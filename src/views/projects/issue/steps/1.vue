    ...
    <div class="content">
      <div class="flex-grow-1 d-flex flex-column">
        <custom-select
          ...
          class="flex-grow-1 danger-indicator"
          ref="cboStage"
        >
          <template slot="label"
  ...
  computed: {
    ...
    stages: {
      get: function () {
        ...
        const projectAllowedStages = [...this.project.stages].map((s) => {
          return s.processStageId;
        });

        const allowedStagesFiltered = this.project.process.stages.filter(
          (ps) => {
            return projectAllowedStages.indexOf(ps.id) > -1;
          }
        );

        return allowedStagesFiltered.map((i) => {
          return {
            value: i.id,
            label: i.title,
          };
