...
export default {
  ...
  computed: {
    ...
    getStartedIdeas: {
      get() {
        ...
        const startedStageIds = this.project.stages.map((x) => x.id);
        const startedIdeas = this.project.userIdeas.filter(
          (x) => {
            // startedStageIds.indexOf(x.stageId) >= 0
            const item = this.ideas.filter((item) => item.id == x.idea.id);

            if( item[0].companyRoleIds == null || !this.role || item[0].companyRoleIds == "undefined" )
              return true;

            return item[0].companyRoleIds.includes(this.role) && startedStageIds.indexOf(x.stageId) >= 0;
          }
        );
        return startedIdeas;
      },
  ...
  async mounted() {
    this.ideas = await this.$store.dispatch("idea/findAll", { force: true});
    ...
    EventBus.$on("invite/changerole", (e) => {
      this.role = e;
    });
    this.loading = false;
  },
  methods: {
    ideaFilter(x) {
      const item = this.ideas.filter((item) => item.id == x.idea.id);
      return item[0].companyRoleIds.includes(this.role);
    },
